...
public class C4 
    ...
    public int h(Tauler t,Tauler asenemy,int color, int col){
            int heuristica = 0;
            ...
            if(t.solucio(col, color)){
                heuristica = 3;
            }else if(asenemy.solucio(col,color*-1)){
                heuristica = -3;
...
//            }
    ...
    public int minimax2 (int profundidad, Tauler t,Tauler asenemy, int color, int True_Color, int col){
        int heuristica = 0;
        asenemy = new Tauler (t);
        asenemy.afegeix(col, color*-1);
        if(profundidad == 0 || t.solucio(col, color) || t.solucio(col, color*-1) || asenemy.solucio(col, color*-1) || asenemy.solucio(col, color) || !t.espotmoure()){
            heuristica= h(t,asenemy,color,col);
        }else{
            if(True_Color == color){ //max
                heuristica= max(t,t, color, profundidad, True_Color);
            }else{ //min
                heuristica= min(t,t, color, profundidad, True_Color);
            }
        } 
        return heuristica;
    }
