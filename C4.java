...
*/    
    ...
    public int moviment(Tauler t, int color) {
       ...
       for(int i = 0; i < t.getMida(); ++i){
           System.out.println("Revisando pos " + i);
           if(t.movpossible(i)){
                heuristica_aux = alphabeta(profundidad_inicial-1,t , color, True_Color, i,alpha, beta);
...
*/    
 ...
 public int AdyacenteVertical(Tauler t, int row, int col, int color, int nivel) {
    ...
    public int AdyacenteHorizontal(Tauler t, int row, int col, int color, int nivel) {
...
public int AdyacenteDiagonal(Tauler t, int row, int col, int color, int nivel) {
...
public int ComprobarAdyacentes(Tauler t, int color, int nivel) {
...
{
    //System.out.println("AAA Profunidad" + profundidad);
    ...
    if(profundidad == 0 || asme.solucio(col, color) || !asme.espotmoure()) {
        ...
        if (True_Color == color)
        {
            ...
            // Recur for left and
            // right children
            for (int i = 0; i < asme.getMida() && asme.movpossible(i); ++i)
            {
                heuristica = alphabeta(profundidad - 1, asme, color*-1, True_Color, i, alpha, beta);
                best = Math.max(best, heuristica);
...
*/  
    public int urgot(Tauler asme, Tauler asenemy, int color, int col){
        ...
        Tauler auxenemy = new Tauler (asenemy);
        auxenemy.afegeix(col, color*-1);
        if(auxenemy.solucio(col, color*-1)){
            avoid_lose=18;
        }
        ...
        return ((win-ifckit)-already_lose) + avoid_lose;
    }
